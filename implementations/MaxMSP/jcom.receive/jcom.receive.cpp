...
void receive_dsp64(TTPtr self, t_object *dsp64, short *count, double samplerate, long maxvectorsize, long flags)
{
	...
	anOutput->mSignalIn->setAttributeValue(TTSymbol("numChannels"), numChannels);
	anOutput->mSignalOut->setAttributeValue(TTSymbol("numChannels"), numChannels);
	anOutput->mSignalTemp->setAttributeValue(TTSymbol("numChannels"), numChannels);
	//anOutput->mSignalZero->setAttributeValue(TTSymbol("numChannels"), numChannels);
	
	anOutput->mSignalIn->setAttributeValue(TTSymbol("vectorSize"), (TTUInt16)maxvectorsize);
	anOutput->mSignalOut->setAttributeValue(TTSymbol("vectorSize"), (TTUInt16)maxvectorsize);
	anOutput->mSignalTemp->setAttributeValue(TTSymbol("vectorSize"), (TTUInt16)maxvectorsize);
	//anOutput->mSignalZero->setAttributeValue(TTSymbol("vectorSize"), (TTUInt16)maxvectorsize);//Do we need zeroSignal?
	
	// mSignalIn will be set in the perform method
	anOutput->mSignalOut->sendMessage(TTSymbol("alloc"));
	anOutput->mSignalTemp->sendMessage(TTSymbol("alloc"));
	//x->zeroSignal->sendMessage(TTSymbol("alloc"));
	//x->zeroSignal->sendMessage(TTSymbol("clear"));
	//audioIn will be set in the perform method
	//x->audioOut->sendMessage(TTSymbol("alloc"));
