...
void hub_autodoc(t_hub *x, t_symbol *msg, long argc, t_atom *argv)
{
	...
	if (argc==1){
		...
		if ((exporttype != gensym("tex")) && (exporttype != gensym("html")) && (exporttype != gensym("textile"))){
    ...
    else if (exporttype == gensym("textile")){
        defer(x, (method)hub_doautodocTextile, userpath, 0, 0L);
        }
    else {
...
void hub_doautodocTextile(t_hub *x, t_symbol *userpath)
{
	...
	// SPECIFY THE FILE WE WANT TO WRITE
	if (!userpath->s_name[0]) {												// Empty string - Throw up a dialog
		snprintf(filename, MAX_FILENAME_CHARS, "%s.textile", x->attr_name->s_name);					// Default File Name
		saveas_promptset("Save Generated Textile Documentation...");			// Instructional Text in the dialog
		err = saveasdialog_extended(filename, &path, &outtype, &type, 1);	// Returns 0 if successful
		if (err)																// User Cancelled
			return;
	}
    ...
    snprintf(tempstring, 1024, "h1. %s", x->attr_name->s_name);
	jcom_core_file_writeline(&file_handle, &myEof, tempstring);
   	jcom_core_file_writeline(&file_handle, &myEof, " ");
    snprintf(tempstring, 1024, "*%s*", x->attr_description->s_name);
	jcom_core_file_writeline(&file_handle, &myEof, tempstring);
	jcom_core_file_writeline(&file_handle, &myEof, " ");
	...
	// Configuration
	jcom_core_file_writeline(&file_handle, &myEof, "h2. Configuration");
	jcom_core_file_writeline(&file_handle, &myEof, " ");
	snprintf(tempstring, 1024, "Module Type: @%s@ ", x->attr_type->s_name);
	jcom_core_file_writeline(&file_handle, &myEof, tempstring);
	snprintf(tempstring, 1024, "Algorithm Type: @%s@ ", x->attr_algorithm_type->s_name);
	jcom_core_file_writeline(&file_handle, &myEof, tempstring);
	snprintf(tempstring, 1024, "Interface Size: @%ld x %ld pixels@", x->attr_size[0], x->attr_size[1]);
	jcom_core_file_writeline(&file_handle, &myEof, tempstring);
	jcom_core_file_writeline(&file_handle, &myEof, " ");
	// Inlets and outlets
	if (x->in_object) {
		...
		argv = NULL;
		object_attr_getvalueof(x->in_object, jps_num_inputs, &argc, &argv);
		snprintf(tempstring, 1024, "Number of signal inlets: @%ld@", atom_getlong(argv));
		jcom_core_file_writeline(&file_handle, &myEof, tempstring);
	...
	if (x->out_object) {	// jcom.out is optional in the case of no signals
		...
		argv = NULL;
		object_attr_getvalueof(x->out_object, jps_num_outputs, &argc, &argv);
		snprintf(tempstring, 1024, "Number of signal outlets: @%ld@", atom_getlong(argv));
		jcom_core_file_writeline(&file_handle, &myEof, tempstring);
		
	}
	else {
		jcom_core_file_writeline(&file_handle, &myEof, "Number of signal outlets: @0@");
	}
    jcom_core_file_writeline(&file_handle, &myEof, " ");
	...
	///////////////////////////////////////////////////
	//jcom_core_file_writeline(&file_handle, &myEof, "\\vspace{1cm}");
	jcom_core_file_writeline(&file_handle, &myEof, "h2. Parameters");
	jcom_core_file_writeline(&file_handle, &myEof, " ");
	table_headingTextile(&file_handle, &myEof, PARAMETER);
	...
	for (i = subscriber->begin(); i != subscriber->end(); ++i) {
		t = *i;
		if (t->type == jps_subscribe_parameter) {
			hub_autodoc_nodeTextile(&file_handle, &myEof, t, PARAMETER);
		}
	}
	critical_exit(0);
	...
	///////////////////////////////////////////////////
	jcom_core_file_writeline(&file_handle, &myEof, " ");
	jcom_core_file_writeline(&file_handle, &myEof, "h2. Messages");
	jcom_core_file_writeline(&file_handle, &myEof, " ");
	table_headingTextile(&file_handle, &myEof, MESSAGE);
	...
	for (i = subscriber->begin(); i != subscriber->end(); ++i) {
		t = *i;
		if (t->type == jps_subscribe_message) {
			hub_autodoc_nodeTextile(&file_handle, &myEof, t, MESSAGE);
		}
	}
	critical_exit(0);
	...
	///////////////////////////////////////////////////
	jcom_core_file_writeline(&file_handle, &myEof, " ");
	jcom_core_file_writeline(&file_handle, &myEof, "h2. Returns");
	jcom_core_file_writeline(&file_handle, &myEof, " ");
	table_headingTextile(&file_handle, &myEof, RETURN);
	...
	for (i = subscriber->begin(); i != subscriber->end(); ++i) {
		t = *i;
		if (t->type == jps_subscribe_return) {
			hub_autodoc_nodeTextile(&file_handle, &myEof, t, RETURN);
		}
	}
	critical_exit(0);
	...
	// Some final info on Jamoma
	jcom_core_file_writeline(&file_handle, &myEof, "h2. About Jamoma ");
   	jcom_core_file_writeline(&file_handle, &myEof, " ");
	jcom_core_file_writeline(&file_handle, &myEof, "Jamoma is a system for creating and exchanging structured Max patches. ");
	jcom_core_file_writeline(&file_handle, &myEof, "It consists of both a set of guidelines and an implementation of those guidelines.");
	jcom_core_file_writeline(&file_handle, &myEof, "For more information please visit http://jamoma.org.");
	// End of page
...
{
	if (nodeType == RETURN){
		jcom_core_file_writeline(file_handle, myEof, "|_. /name |_. /type |_. /range |_. /range |_. /ramp |_. /ramp |_. /dataspace |_. /dataspace |_. /repetitions |_. /enable |_. /description | ");
		jcom_core_file_writeline(file_handle, myEof, "|_.  |_.       |_. /bounds |_.  /clipmode |_. /drive |_. /function |_. |_. /unit |_. /allow |_. |_. | ");
    }
	else{
		jcom_core_file_writeline(file_handle, myEof, "|_. /name |_. /type |_. /range |_. /range |_. /ramp |_. /ramp  |_. /dataspace |_. /dataspace |_. /repetitions |_. /description |");
		jcom_core_file_writeline(file_handle, myEof, "|_.       |_.       |_. /bounds |_. /clipmode |_. /drive |_.  /function |_.  |_. /unit |_. /allow |_. |");
    }
}
