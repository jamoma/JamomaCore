...
TTCueManager::~TTCueManager()
{
	...
	for (i = 0; i < names.size(); i++) {
		...
		oldCue = TTCuePtr((TTObjectPtr)v[0]);
...
TTErr TTCueManager::Prepare(const TTValue& inputValue, TTValue& outputValue)
{
	TTValue		v;
	...
	if (!mCues->lookup(mCurrent, v)) {
		...
		if (mCurrentCue)
			return mCurrentCue->sendMessage(TTSymbol("Prepare"), anAddress, kTTValNONE);
	}
...
TTErr TTCueManager::Interpolate(const TTValue& inputValue, TTValue& outputValue)
{
	...
	if (inputValue.size() == 3) {
        ...
        if (!mCues->lookup(name1, v1) && !mCues->lookup(name2, v2)) {
            ...
            cue1 = TTCuePtr((TTObjectPtr)v1[0]);
            cue2 = TTCuePtr((TTObjectPtr)v2[0]);
	...
	for (i = 1; i < inputValue.size(); i++) {
		...
		if (inputValue[i-1].type() == kTypeSymbol && inputValue[i].type() == kTypeSymbol) {
			...
			if (!mCues->lookup(nameToMerge, v))
				aCueToMerge = TTCuePtr((TTObjectPtr)v[0]);
			
			aCueToOptimize = NULL;
			if (!mCues->lookup(nameToOptimize, v))
				aCueToOptimize = TTCuePtr((TTObjectPtr)v[0]);
