...
#ifndef __TT_BUFFER_HANDLER_H__
#define __TT_BUFFER_HANDLER_H__
...
class TTMODULAR_EXPORT TTBufferHandler : public TTDataObject
{
	TTCLASS_SETUP(TTBufferHandler)
...
public:	// use public for recursive access
	...
	TTString			*mWriter;						///< the buffer where to write
	TTString			*mReader;						///< the buffer to read
	...
	/** TTBufferWriter could takes a size or nothing.
		In the size case, TTBufferWriter creates a buffer and 
		then calls the WriteInBuffer method of mObject attribute
		In the second case, it directly calls the WriteInBuffer method */
	TTErr Write(const TTValue& args, TTValue& outputValue);
	
	/** TTBufferReader could takes a buffer pointer or nothing.
		In the buffer pointer case, TTBufferReader starts buffer reading and 
		then calls the ReadFromBuffer method of mObject attribute
		In the second case, it directly calls the ReadFromBuffer method */
...
typedef TTBufferHandler* TTBufferHandlerPtr;


#endif // __TT_BUFFER_HANDLER_H__
