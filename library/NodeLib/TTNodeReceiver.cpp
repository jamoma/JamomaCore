...
#include "TTNodeReceiver.h"

#define thisTTClass			TTNodeReceiver
#define thisTTClassName		"NodeReceiver"
...
TTNodeReceiver::~TTNodeReceiver()
{;}

TTErr TTNodeReceiver::setAddress(const TTValue& newValue)
{
	unbind();
	mAddress = newValue;
	bind();
...
TTErr TTNodeReceiver::setAttribute(const TTValue& newValue)
{
	mAttribute = newValue;
...
TTErr TTNodeReceiver::setEnable(const TTValue& newValue)
{
	mEnable = newValue;
...
TTErr TTNodeReceiver::bind()
{
	...
	if ((mAttribute != TT("created")) && (mAttribute != TT("destroyed")) && (mAttribute != TT("initialized")))
	{
		...
		if (!err) {
			...
			for (mNodesCache->begin(); mNodesCache->end(); mNodesCache->next())
			{
				...
				if (!err) {
					...
					newObserver->setAttributeValue(TT("Function"), TTPtr(&NodeReceiverAttrCallback));
		...
		mObserver->setAttributeValue(TT("Function"), TTPtr(&NodeReceiverDirectoryCallback));
		
		mObserver->setAttributeValue(TT("Owner"), TT("TTNodeReceiver"));							// this is usefull only to debug
...
TTErr TTNodeReceiver::directoryNotification(TTPtr baton, TTValue& data)
{
	TTSymbolPtr		oscAddress;
	TTAttributePtr	anAttribute = NULL;
...
TTErr TTNodeReceiver::attrNotification(TTPtr baton, TTValue& data)
{
	if(mEnable) {
...
TTErr NodeReceiverDirectoryCallback(TTPtr baton, TTValue& data)
{
	TTValuePtr			b;
	TTNodeReceiverPtr	aNodeReceiver;
	TTSymbolPtr			aSymbol;
...
TTErr NodeReceiverAttrCallback(TTPtr baton, TTValue& data)
{
	TTValuePtr			b;
	TTNodeReceiverPtr	aNodeReceiver;
	TTSymbolPtr			aSymbol;
