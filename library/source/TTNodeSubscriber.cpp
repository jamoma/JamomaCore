...
#define thisTTClass			TTNodeSubscriber
#define thisTTClassName		"NodeSubscriber"
...
mDirectory(NULL), mContext(NULL), mRelativeAddress(kTTSymEmpty), mShareCallback(NULL), mContextListCallback(NULL)
{
	...
	TT_ASSERT("Correct number of args to create TTNodeSubscriber", arguments.getSize() == 6);
	
	arguments.get(0, (TTPtr*)&mDirectory);
	TT_ASSERT("Directory passed to TTNodeSubscriber is not NULL", mDirectory);
	arguments.get(1, &mContext);
	TT_ASSERT("Context passed to TTNodeSubscriber is not NULL", mContext);
	
	arguments.get(2, &mRelativeAddress);
	
	arguments.get(3, (TTPtr*)&mShareCallback);
	TT_ASSERT("Share Callback passed to TTNodeSubscriber is not NULL", mShareCallback);
	arguments.get(4, (TTPtr*)&mContextListCallback);
	TT_ASSERT("ContextList Callback passed to TTNodeSubscriber is not NULL", mContextListCallback);
	
	arguments.get(5, (TTPtr*)&anObject);
	
	addAttribute(Context, kTypePointer);
	addAttribute(RelativeAddress, kTypeSymbol);
	addAttribute(AbsoluteAddress, kTypeSymbol);
	addAttribute(ContextAddress, kTypeSymbol);
	addAttribute(Node, kTypeObject);
	addAttribute(ContextNode, kTypeObject);
	
	this->subscribe(anObject);
}

TTNodeSubscriber::~TTNodeSubscriber()
{
...
/*
TTErr TTNodeSubscriber::getContext(TTValue& value)
{
	value = mContext;
	return kTTErrNone;
}

TTErr TTNodeSubscriber::getRelativeAddress(TTValue& value)
{
	value = mRelativeAddress;
	return kTTErrNone;
}

TTErr TTNodeSubscriber::getAbsoluteAddress(TTValue& value)
{
	value = mAbsoluteAddress;
	return kTTErrNone;
}

TTErr TTNodeSubscriber::getContextAddress(TTValue& value)
{
	value = mContextAddress;
	return kTTErrNone;
}

TTErr TTNodeSubscriber::getNode(TTValue& value)
{
	value = mNode;
	return kTTErrNone;
}

TTErr TTNodeSubscriber::getContextNode(TTValue& value)
{
	value = mContextNode;
	return kTTErrNone;
}
*/
TTErr TTNodeSubscriber::subscribe(TTObjectPtr anObject)
{
	...
	if (!this->mContextNode) {
		...
		this->subscribeContextList(aContextList);
	}
	...
	if (this->mContextNode) {
		...
		if (err) {
			...
			if (_parent != NO_PARENT) {
				...
				if (!err)
					this->mDirectory->TTNodeCreate(TT(nodeAddress.data()), NULL, this->mContextNode->getContext(), &aParentNode, &newInstanceCreated);
			}
...
TTErr TTNodeSubscriber::subscribeContextList(TTListPtr aContextList)
{
	TTValue			aTempValue, args;
