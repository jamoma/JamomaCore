...
TTAttribute::TTAttribute(const TTSymbolRef newName, TTDataType newType, void* newAddress) : 
	TTObject(kTTValNONE), 
	name(newName), 
	type(newType), 
	address(newAddress), 
...
TTAttribute::TTAttribute(const TTSymbolRef newName, TTDataType newType, void* newAddress, TTGetterMethod newGetter)
: TTObject(kTTValNONE), name(newName), type(newType), address(newAddress), getter(newGetter), getterObject(NULL), setterObject(NULL), getterFlags(kTTAttrDefaultFlags), setterFlags(kTTAttrDefaultFlags),
  readOnly(NO), rangeLowBound(0.0), rangeHighBound(1.0), rangeChecking(TT("none")), hidden(NO), description(kTTSymEmpty)
{
	setter = (TTSetterMethod)&TTAttribute::defaultSetter;
}

TTAttribute::TTAttribute(const TTSymbolRef newName, TTDataType newType, void* newAddress, TTSetterMethod newSetter)
...
TTAttribute::TTAttribute(const TTSymbolRef newName, TTDataType newType, void* newAddress, TTGetterMethod newGetter, TTSetterMethod newSetter)
...
TTAttribute::TTAttribute(const TTSymbolRef newName, const TTObjectPtr newGetterObject, const TTObjectPtr newSetterObject)
	...
	switch(attribute.type) {
		...
		case kTypeBoolean:
			*((TTBoolean*)attribute.address) = value;
			return kTTErrNone;
		case kTypeSymbol:
			*((TTSymbol*)attribute.address) = (TTSymbolRef)value;
			return kTTErrNone;
		case kTypeObject:
...
TTErr TTAttribute::extendedSetter(const TTAttribute& attribute, TTValue& value)
{
	...
	return anExtendedObject->setAttributeValue((TTSymbolRef)attribute.name, value);
}
