 ...
 *
 * 1. Absolutely flat amplitude response throughout the passband with a 12 dB/octave rolloff rate after the crossover point. 
 ...
 *
 */
class TTLowpassLinkwitzRiley2 : public TTAudioObjectBase {
	TTCLASS_SETUP(TTLowpassLinkwitzRiley2)

protected:

	TTFloat64			mFrequency;				///< filter cutoff frequency
	TTFloat64			mK, mRadians, mKSquared, mRadiansSquared;		
	TTFloat64			mA0, mA1, mA2;					///< filter coefficients for input samples
	TTFloat64			mB1, mB2;					///< filter coefficients for output samples
...
#endif // __TT_LOWPASS_LINKWITZ_RILEY_2_H__
