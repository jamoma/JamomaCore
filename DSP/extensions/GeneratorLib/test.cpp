...
extern "C" TTErr TTLoadJamomaExtension_GeneratorLib(void);               // CUSTOMIZE

int main(int argc, const char * argv[])
{
	std::cout << "BEGIN testing for GeneratorLib...\n";                  // CUSTOMIZE
    TTLoadJamomaExtension_GeneratorLib();                                // CUSTOMIZE
	
	TTValue classNames;
	
	// if the follow group tag is present within the thisTTClassTags definition, the class will be tested
    TTValue testClassesWithTheseTags(TT("dspGeneratorLib"));             // CUSTOMIZE
	...
	std::cout << "END testing of GeneratorLib!\n";                       // CUSTOMIZE
    return 0;
}
