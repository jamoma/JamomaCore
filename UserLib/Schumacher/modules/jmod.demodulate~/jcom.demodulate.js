...
setinletassist(0,"Number of Demodulators, dspstate");
...
function msg_int(value)
{
    ...
    if (DSP_running==1)
    {
        ...
        post("jmod.demodulate~: Audio is currently running.");
        post();
        post("Number of demodulators will be updated next time audio is turned off.");
        post();
    }
    ...
    {    
        ...
        if (num_voices==num_voices_prev) return;
         ...
         for (i=num_voices_prev; i>value; i--)
                {
            outlet(0, "script", "delete", "demodulator["+(i+0)+"]");
        	}
        	
          for (i=num_voices_prev; i<value; i++)     
        {
            outlet(0, "script", "newdefault", "demodulator["+(i+1)+"]", (50+25*i), (120+20*i), "jcom.demodulate~");
            outlet(0, "script", "connect", "sigin", 0, "demodulator["+(i+1)+"]", 0);
            outlet(0, "script", "connect", "demodulator["+(i+1)+"]", 0, "out["+(i+1)+"]",0);
            outlet(0, "script", "connect", "route", i, "demodulator["+(i+1)+"]", 0);
        }
        outlet(0, "done");
    }
}