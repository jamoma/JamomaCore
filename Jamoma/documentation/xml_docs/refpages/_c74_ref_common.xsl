  ...
  <xsl:template match="c74object">
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <!-- box attr link -->
        <xsl:if test="/c74object/@name != 'JamomaDataspaceLib' and /c74object/@name != 'JamomaFunctionLib' and /c74object/@name != 'JamomaRampLib'" >	
          <h4>
            <a>
...
<!-- should not be shown -->
            ...
            <xsl:if test="attributelist/attribute[@name='default']">
              <tr>
                <td/>
                <td class="defaultval" colspan="2">def.:<xsl:value-of select="attributelist/attribute[@name='default']/@value"/></td>
              </tr>
            </xsl:if>
          </xsl:if>
        </xsl:for-each>
      ...
      <xsl:when test="arg">
        <td>
          <xsl:for-each select="arg">
            <span class="arglist_arg"><xsl:value-of select="@name"/> [<xsl:value-of select="@type"/>]</span>
          </xsl:for-each>
        </td>
      </xsl:when>
      <xsl:otherwise>
