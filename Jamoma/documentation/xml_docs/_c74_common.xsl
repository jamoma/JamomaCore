...
<!-- 
  ...
  <xsl:template name="refpage">
    ...
    <xsl:variable name="objectname"><xsl:value-of select="$refname"/>.maxref.xml</xsl:variable>
    <a>
      <xsl:attribute name="href">
        <xsl:variable name="temp">
          <xsl:for-each select="document(concat($refs_root_loc,'/_c74_ref_modules.xml'))/root/module">
            <xsl:if test="document(concat($refs_root_loc,'/',normalize-space(.),'/_c74_contents.xml'))/root/refpage[@name=$objectname]">
              <xsl:value-of select="concat($refs_root_link,'/',normalize-space(.),'/',$objectname)"/> 
            </xsl:if>
          </xsl:for-each>
        </xsl:variable>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:choose>
              <xsl:when test="$kernel='1' and document(concat($refs_root_loc,'/kernel-ref/_c74_contents.xml'))/root/refpage[@name=$objectname]"> 
                <xsl:value-of select="concat($refs_root_link,'/kernel-ref/', $objectname)"/> 
              </xsl:when> 
        ...
        <!-- <xsl:choose>
          <xsl:when test="document(concat($refs_root_loc,'/max-ref/_c74_contents.xml'))/root/refpage[@name=$objectname]"> 
            <xsl:value-of select="concat($refs_root_link,'/max-ref/',$objectname)"/> 
          </xsl:when> 
          <xsl:when test="document(concat($refs_root_loc,'/msp-ref/_c74_contents.xml'))/root/refpage[@name=$objectname]"> 
            <xsl:value-of select="concat($refs_root_link,'/msp-ref/',$objectname)"/> 
          </xsl:when> 
          <xsl:when test="document(concat($refs_root_loc,'/jit-ref/_c74_contents.xml'))/root/refpage[@name=$objectname]">  
            <xsl:value-of select="concat($refs_root_link,'/jit-ref/', $objectname)"/>  
          </xsl:when>  
          <xsl:when test="$kernel='1' and document(concat($refs_root_loc,'/kernel-ref/_c74_contents.xml'))/root/refpage[@name=$objectname]"> 
            <xsl:value-of select="concat($refs_root_link,'/kernel-ref/', $objectname)"/> 
          </xsl:when> 
  ...
  <xsl:template name="object_link">
    ...
    <xsl:variable name="refname">
      <xsl:call-template name="name_replace">
        <xsl:with-param name="name" select="$oname"/>
      </xsl:call-template>
    </xsl:variable>
	...
	<xsl:template name="handle_link">
	  ...
	  <xsl:choose>
       ...
       <xsl:variable name="hash">
	    ...
	    <a>
	      <xsl:attribute name="href">
	        <xsl:value-of select="concat($baseloc, $module, $linkname, $suffix, $hash)"/>
	      </xsl:attribute>
	      <xsl:value-of select="normalize-space(.)" />
	  </a>
	  </xsl:otherwise>
