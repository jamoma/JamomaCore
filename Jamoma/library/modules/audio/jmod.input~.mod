max v2;
#N vpatcher 7 46 1017 743;
#P window setfont "Sans Serif" 9.;
#P newex 8 301 174 196617 jmod.hub.mxt $0_ jmod.input~ @size 2U-half @module_type audio @num_inputs 0 @num_outputs 2 @options no_panel;
#N vpatcher 34 89 634 489;
#P outlet 80 238 15 0;
#P window setfont "Sans Serif" 9.;
#P newex 80 200 35 196617 zl join;
#P newex 105 174 21 196617 t 0;
#P inlet 80 117 15 0;
#P comment 51 43 212 196617 append doesn't fancy "set" messages as they are used to reset what is to be appended. Hence this hack to append 0 to the incoming message.;
#P connect 1 0 3 0;
#P connect 3 0 4 0;
#P connect 1 0 2 0;
#P connect 2 0 3 1;
#P pop;
#P newobj 286 184 73 196617 p append_hack;
#N vpatcher 34 89 634 489;
#P outlet 80 238 15 0;
#P window setfont "Sans Serif" 9.;
#P newex 80 200 35 196617 zl join;
#P newex 105 174 21 196617 t 0;
#P inlet 80 117 15 0;
#P comment 51 43 212 196617 append doesn't fancy "set" messages as they are used to reset what is to be appended. Hence this hack to append 0 to the incoming message.;
#P connect 1 0 3 0;
#P connect 3 0 4 0;
#P connect 1 0 2 0;
#P connect 2 0 3 1;
#P pop;
#P newobj 473 184 73 196617 p append_hack;
#P newex 835 166 65 196617 prepend open;
#P user jmod.meter~ 35 28 123 10;
#P user jmod.meter~ 35 44 123 10;
#P number 379 403 35 9 0 0 0 3 0 0 0 74 156 97 222 222 222 0 0 0;
#P number 382 506 35 9 0 0 0 3 0 0 0 74 156 97 222 222 222 0 0 0;
#P newex 521 613 100 196617 pvar filename_menu;
#B color 7;
#P user umenu 192 87 63 196647 3 64 103 0;
#X setrgb 198 198 198 221 221 221 255 255 255 221 221 221 170 170 170 119 119 119 187 187 187;
#P objectname filename_menu;
#P newex 521 590 98 196617 r $0_xxx_filename;
#B color 14;
#P user led 338 63 17 17 1 150;
#P user pictctrl 5 71 14 13 jmod.button.link_toggle.pct 0 13 1 1 26 0 1 0 14 13 128 0 1 10 2 1;
#P objectname Link;
#P newex 286 119 63 196617 prepend link;
#B color 11;
#P newex 286 63 51 196617 pvar Link;
#B color 7;
#P newex 286 84 143 196617 jmod.parameter.mxt $0_ link;
#B color 2;
#P objectname jmod.parameter.mxt[2];
#N js jmod.js_link.js 2;
#P newobj 286 157 197 196617 js jmod.js_link.js 2;
#B color 11;
#P newex 727 166 35 196617 t open;
#P newex 730 229 80 196617 r $0_xxx_done;
#B color 14;
#P newex 473 262 90 196617 s $0_xxx_gain_2;
#P newex 286 262 90 196617 s $0_xxx_gain_1;
#P message 732 559 45 196617 set \$1 0;
#P newex 732 623 78 196617 s $0_xxx_gain;
#P number 823 579 35 9 0 0 0 3 0 0 0 74 156 97 222 222 222 0 0 0;
#P number 524 206 35 9 0 0 0 3 0 0 0 74 156 97 222 222 222 0 0 0;
#P number 338 206 35 9 0 0 0 3 0 0 0 74 156 97 222 222 222 0 0 0;
#P newex 286 206 49 196617 pvar ch1;
#B color 7;
#P user pictslider 20 98 161 13 2 0 2 0 jmod.slider.master.knob.top.pct 1 jmod.slider.master.bg.pct 1 609 0 8323072 157 1. 1.;
#P objectname gain_master;
#P newex 473 206 49 196617 pvar ch2;
#B color 7;
#P newex 732 578 89 196617 pvar gain_master;
#B color 7;
#P newex 473 228 178 196617 jmod.parameter.gain.mxt $0_ gain_2;
#B color 2;
#P objectname jmod.parameter.gain.mxt[2];
#P newex 286 228 178 196617 jmod.parameter.gain.mxt $0_ gain_1;
#B color 2;
#P objectname jmod.parameter.gain.mxt[1];
#P newex 732 599 165 196617 jmod.parameter.gain.mxt $0_ gain;
#B color 2;
#P objectname jmod.parameter.gain.mxt;
#P user jsui 195 24 45 19 1 0 0 jsui_textbutton.js Open;
#P objectname Open_button;
#P user jsui 195 47 45 19 1 0 0 jmod.jsui_texttoggle.js Play Play 204 204 204 0 0 0 121 255 15;
#P objectname Play_button;
#P window setfont "Sans Serif" 12.;
#P comment 286 353 210 196620 DSP and adc~ settings;
#B color 7;
#P window setfont "Sans Serif" 9.;
#P newex 286 602 35 196617 t open;
#P newex 286 579 85 196617 pvar DSP_Status;
#B color 7;
#P newex 286 623 75 196617 s $0_xxx_adc;
#B color 14;
#P message 286 535 45 196617 set 2 \$1;
#P message 286 431 45 196617 set 1 \$1;
#P newex 286 553 75 196617 s $0_xxx_adc;
#B color 14;
#P newex 286 450 75 196617 s $0_xxx_adc;
#B color 14;
#P newex 286 505 95 196617 pvar Channel_Right;
#B color 7;
#P newex 286 402 91 196617 pvar Channel_Left;
#B color 7;
#P newex 286 485 169 196617 jmod.parameter.mxt $0_ set_ch_2;
#B color 2;
#P objectname jmod.parameter.mxt[1];
#P newex 286 382 169 196617 jmod.parameter.mxt $0_ set_ch_1;
#B color 2;
#P objectname jmod.parameter.mxt;
#P user jsui 196 0 45 19 1 0 0 jsui_textbutton.js DSP;
#P objectname DSP_Status;
#P number 7 43 30 9 1 512 8227 3 198 198 198 221 221 221 222 222 222 0 0 0;
#P objectname Channel_Right;
#P number 7 26 30 9 1 512 8227 3 198 198 198 221 221 221 222 222 222 0 0 0;
#P objectname Channel_Left;
#P toggle 85 425 15 0;
#P message 47 425 35 196617 1 100;
#P message 8 425 35 196617 0 100;
#P message 8 509 69 196617 \; max refresh;
#P message 8 477 72 196617 \; jmod.init bang;
#P newex 8 449 84 196617 s jmod.dsptoggle;
#P window setfont "Sans Serif" 12.;
#P comment 286 7 210 196620 Volume control;
#B color 7;
#P comment 667 492 210 196620 All communication with audio subpatch is done using $0_xxx_ messages;
#B color 14;
#P comment 667 340 39 196620 loop;
#B color 7;
#P comment 667 258 39 196620 play;
#B color 7;
#P comment 667 142 42 196620 open;
#B color 7;
#P comment 667 65 56 196620 source;
#B color 7;
#P window setfont "Sans Serif" 9.;
#P number 822 86 35 9 0 0 0 3 0 0 0 74 156 97 222 222 222 0 0 0;
#P user led 816 142 17 17 1 150;
#P user led 816 360 17 17 1 150;
#P user led 816 276 17 17 1 150;
#P newex 152 183 94 196617 pvar Time_Display;
#N vpatcher 355 70 1247 796;
#P origin -88 11;
#P window setfont "Sans Serif" 9.;
#P message 376 131 56 196617 modout 1;
#P newex 376 111 55 196617 r jmod.init;
#P newex 457 77 54 196617 route loop;
#N vpatcher 191 93 619 557;
#P window setfont "Sans Serif" 9.;
#P newex 237 275 55 196617 r jmod.init;
#P newex 237 324 39 196617 t clear;
#P newex 159 349 98 196617 s $0_xxx_filename;
#P newex 159 323 75 196617 prepend append;
#P newex 159 277 50 196617 strippath;
#P outlet 65 312 15 0;
#P inlet 65 144 15 0;
#P newex 65 285 65 196617 prepend open;
#P newex 65 242 83 196617 opendialog sound;
#P newex 65 187 104 196617 sel bang;
#P comment 39 62 332 196617 By using opendialog it's made sure that any opendialog message that's reaching sfplay~ also has a filename argument.;
#P comment 39 90 332 196617 That way we know for certain that any "open" message reaching sfplay~ implies that a new file is to be opened imideately. Hence the Play button can be turned of instantly.;
#P comment 39 43 332 196617 The "Play" GUI button should be turned of when a new file is opened.;
#P connect 6 0 3 0;
#P connect 3 0 4 0;
#P connect 4 0 5 0;
#P fasten 3 1 5 0 164 259;
#P connect 5 0 7 0;
#P connect 3 1 8 0;
#P connect 4 0 8 0;
#P connect 8 0 9 0;
#P connect 9 0 10 0;
#P connect 11 0 10 0;
#P connect 12 0 11 0;
#P connect 8 0 11 0;
#P pop;
#P newobj 275 96 65 196617 p open_stuff;
#P newex 671 303 65 196617 prepend open;
#P newex 671 274 83 196617 opendialog sound;
#P newex 671 252 104 196617 sel bang;
#P comment 447 615 332 196617 By using opendialog it's made sure that any opendialog message that's reaching sfplay~ also has a filename argument.;
#P comment 447 643 332 196617 That way we know for sure that any "open" message reaching sfplay~ for sure implies that a new file is to be opened imideately \, and hence the Play button can be turned of instantly.;
#P comment 447 596 332 196617 The "Play" GUI button should be turned of when a new file is opened.;
#P newex 457 100 30 196617 == 0;
#P newex 457 234 80 196617 s $0_xxx_done;
#B color 14;
#P newex 457 213 21 196617 t 0;
#P newex 457 186 27 196617 gate;
#P newex 275 72 81 196617 route open;
#P newex 172 199 75 196617 r $0_xxx_adc;
#B color 14;
#N vpatcher 480 147 923 501;
#P window setfont "Sans Serif" 9.;
#P comment 74 301 202 196617 NOTE: This fade is not displayed in the GUI.;
#P comment 74 84 145 196617 Quick fade in when audio start;
#P inlet 329 58 15 0;
#P inlet 264 58 15 0;
#P outlet 296 272 15 0;
#P outlet 264 272 15 0;
#P newex 264 231 75 196617 matrix~ 2 2 0.;
#P message 106 198 75 196617 0 0 \$1 \, 1 1 \$1;
#P newex 106 162 60 196617 unpack 0 0.;
#P message 191 198 45 196617 ramp \$1;
#P newex 191 162 21 196617 t 0;
#P newex 106 122 84 196617 r jmod.dsptoggle;
#P comment 74 68 145 196617 Quick fade out when audio stop;
#P connect 1 0 4 0;
#P connect 4 0 5 0;
#P connect 1 0 2 0;
#P connect 4 1 3 0;
#P connect 2 0 3 0;
#P connect 9 0 6 0;
#P connect 5 0 6 0;
#P connect 3 0 6 0;
#P connect 6 0 7 0;
#P connect 6 1 8 0;
#P connect 10 0 6 1;
#P pop;
#P newobj 20 499 84 196617 p smooth_in_out;
#P comment 423 505 85 196617 Time marker;
#P comment 114 548 54 196617 signal R;
#P comment 39 548 54 196617 signal L;
#N comlet signal L;
#P outlet 20 548 15 0;
#N comlet signal R;
#P outlet 94 548 15 0;
#N comlet Time marker;
#P outlet 406 505 15 0;
#N vpatcher 497 274 1063 787;
#P window setfont "Sans Serif" 9.;
#P newex 88 46 170 196617 scale 0. 158. 0. 7.94321 1.071519;
#P newex 192 100 170 196617 scale 0. 158. 0. 7.94321 1.071519;
#P newex 214 181 170 196617 scale 0. 158. 0. 7.94321 1.071519;
#P newex 214 159 78 196617 r $0_xxx_gain;
#B color 14;
#P newex 192 274 27 196617 * 0.;
#P newex 88 274 27 196617 * 0.;
#P newex 58 221 20 196617 t b;
#P newex 192 78 90 196617 r $0_xxx_gain_2;
#P newex 88 24 90 196617 r $0_xxx_gain_1;
#P comment 262 48 100 196617 MIDI to amplitude;
#P comment 431 429 54 196617 signal R;
#P comment 387 429 54 196617 signal L;
#P comment 456 33 54 196617 signal R;
#N comlet signal R;
#P outlet 435 406 15 0;
#N comlet signal L;
#P outlet 403 406 15 0;
#N comlet signal R;
#P inlet 468 50 15 0;
#N comlet signal L;
#P inlet 403 50 15 0;
#P newex 403 356 75 196617 matrix~ 2 2 0.;
#P message 88 297 39 196617 0 0 \$1;
#P message 192 297 39 196617 1 1 \$1;
#P comment 384 33 54 196617 signal L;
#P connect 18 0 14 0;
#P connect 12 0 20 0;
#P connect 14 0 15 0;
#P connect 20 0 15 0;
#P connect 15 0 2 0;
#P connect 18 0 15 1;
#P connect 13 0 19 0;
#P connect 14 0 16 0;
#P connect 19 0 16 0;
#P connect 16 0 1 0;
#P connect 18 0 16 1;
#P connect 17 0 18 0;
#P connect 4 0 3 0;
#P fasten 1 0 3 0 197 322 408 322;
#P fasten 2 0 3 0 93 322 408 322;
#P connect 3 0 6 0;
#P connect 3 1 7 0;
#P connect 5 0 3 1;
#P pop;
#P newobj 20 468 84 196617 p gain;
#P newex 275 40 88 196617 r $0_xxx_sfplay;
#B color 14;
#P newex 20 199 98 196617 r $0__xxx_sfmenu;
#B color 14;
#P number 20 335 35 9 0 0 0 3 0 0 0 221 221 221 222 222 222 0 0 0;
#P hidden comment 181 653 105 196617 ----signal inputs----;
#P newex 559 303 51 196617 *~ 0.125;
#P newex 559 276 65 196617 cycle~ 1000;
#P newex 635 276 32 196617 pink~;
#N vpatcher 177 193 878 686;
#P window setfont "Sans Serif" 9.;
#P newex 30 320 45 196617 split 0 9;
#P newex 30 342 60 196617 sprintf O%i;
#P newex 30 366 49 196617 tosymbol;
#P newex 137 342 60 196617 sprintf O%i;
#P newex 137 366 49 196617 tosymbol;
#P newex 137 320 45 196617 split 0 9;
#P newex 244 342 60 196617 sprintf O%i;
#P newex 244 372 49 196617 tosymbol;
#P newex 244 320 45 196617 split 0 9;
#P outlet 30 447 15 0;
#P newex 30 398 225 196617 sprintf %s:%s:%s;
#P newex 30 422 89 196617 prepend setitem 0;
#P newex 164 162 68 196617 expr $f1-$i2;
#P newex 244 186 33 196617 * 60.;
#P newex 46 114 68 196617 expr $f1-$i2;
#P window setfont "Sans Serif" 36.;
#P number 244 261 62 36 0 0 32 3 0 0 0 221 221 221 222 222 222 0 0 0;
#P comment 310 261 38 196644 s;
#P number 137 261 62 36 0 0 32 3 0 0 0 221 221 221 222 222 222 0 0 0;
#P window setfont "Sans Serif" 9.;
#P newex 137 139 33 196617 * 60.;
#P window setfont "Sans Serif" 36.;
#P comment 201 261 38 196644 m;
#P number 30 261 62 36 0 0 32 3 0 0 0 221 221 221 222 222 222 0 0 0;
#P window setfont "Sans Serif" 9.;
#P newex 30 91 62 196617 / 3600000.;
#P newex 30 71 80 196617 snapshot~ 1000;
#P inlet 30 27 15 0;
#P window setfont "Sans Serif" 36.;
#P comment 94 261 28 196644 h;
#P connect 1 0 2 0;
#P connect 2 0 3 0;
#P connect 3 0 4 0;
#P connect 4 0 24 0;
#P connect 24 0 23 0;
#P connect 24 1 22 0;
#P connect 23 0 22 0;
#P connect 22 0 14 0;
#P connect 14 0 13 0;
#P connect 13 0 15 0;
#P fasten 3 0 10 0 35 111 51 111;
#P fasten 3 0 10 1 35 110 109 110;
#P fasten 10 0 6 0 51 135 142 135;
#P connect 6 0 7 0;
#P connect 7 0 19 0;
#P connect 19 0 21 0;
#P connect 21 0 20 0;
#P connect 19 1 20 0;
#P connect 20 0 14 1;
#P fasten 6 0 12 0 142 158 169 158;
#P fasten 6 0 12 1 142 158 227 158;
#P fasten 12 0 11 0 169 183 249 183;
#P connect 11 0 9 0;
#P connect 9 0 16 0;
#P connect 16 0 18 0;
#P connect 16 1 17 0;
#P connect 18 0 17 0;
#P connect 17 0 14 2;
#P pop;
#P newobj 406 362 59 196617 p h:m:s:ms;
#N sfplay~  2 403200 1 ;
#P newobj 346 156 100 196617 sfplay~ 2 403200 1;
#P newex 94 427 66 196617 selector~ 4;
#P newex 20 427 66 196617 selector~ 4;
#P newex 172 311 29 196617 adc~;
#P connect 10 0 9 0;
#P connect 9 0 1 0;
#P connect 1 0 12 0;
#P connect 12 0 19 0;
#P connect 19 0 15 0;
#P fasten 0 0 1 1 177 381 39 381;
#P fasten 3 0 1 2 351 385 53 385;
#P fasten 7 0 1 3 564 388 67 388;
#P fasten 5 0 1 4 640 391 81 391;
#P connect 9 0 2 0;
#P connect 2 0 12 1;
#P connect 12 1 19 1;
#P connect 19 1 14 0;
#P fasten 0 1 2 1 196 410 113 410;
#P fasten 3 1 2 2 381 413 127 413;
#P fasten 7 0 2 3 564 416 141 416;
#P fasten 5 0 2 4 640 419 155 419;
#P connect 20 0 0 0;
#P connect 11 0 21 0;
#P connect 21 0 32 0;
#P connect 32 0 3 0;
#P connect 21 1 3 0;
#P connect 35 0 3 0;
#P connect 34 0 35 0;
#P connect 3 2 4 0;
#P connect 4 0 13 0;
#P fasten 11 0 33 0 280 62 462 62;
#P connect 33 0 25 0;
#P connect 25 0 22 0;
#P fasten 32 0 23 0 280 210 462 210;
#P connect 22 0 23 0;
#P connect 23 0 24 0;
#P connect 3 3 22 1;
#P connect 6 0 7 0;
#P connect 29 0 30 0;
#P fasten 29 1 31 0 770 298 676 298;
#P connect 30 0 31 0;
#P pop;
#P newobj 78 160 84 196617 p Sound_sources;
#P newex 727 142 88 196617 pvar Open_button;
#B color 7;
#P newex 730 462 88 196617 s $0_xxx_sfplay;
#B color 14;
#P newex 727 196 88 196617 s $0_xxx_sfplay;
#P message 730 392 42 196617 loop \$1;
#P newex 730 411 88 196617 s $0_xxx_sfplay;
#B color 14;
#P newex 730 360 85 196617 pvar Loop_toggle;
#B color 7;
#P newex 730 309 88 196617 s $0_xxx_sfplay;
#B color 14;
#P newex 730 276 85 196617 pvar Play_button;
#B color 7;
#P newex 727 112 98 196617 s $0__xxx_sfmenu;
#B color 14;
#P newex 727 85 93 196617 pvar input_menu 2;
#B color 7;
#P newex 835 143 153 196617 jmod.message.mxt $0_ sf_open;
#B color 2;
#P objectname jmod.parameter.mxb[10];
#P user umenu 81 1 100 196645 1 64 17 0;
#X add No Input;
#X add Live Input;
#X add Soundfile;
#X add Test Tone: 1KHz;
#X add Test Source: Pink Noise;
#P objectname input_menu;
#P newex 188 306 81 196617 pattrstorage @autorestore 0 @savemode 0;
#X client_rect 54 290 488 814;
#X storage_rect 0 0 640 240;
#P objectname jmod.input~;
#P user pictslider 20 72 161 13 2 0 2 0 jmod.slider.ch.knob.top.pct 1 jmod.slider.ch.bg.pct 1 609 0 0 157 1. 1.;
#P objectname ch1;
#P user pictslider 20 85 161 13 2 0 2 0 jmod.slider.ch.knob.bottom.pct 1 jmod.slider.ch.bg.pct 1 609 0 8323072 157 1. 1.;
#P objectname ch2;
#P user fpic 26 18 160 52 jmod.bg.hgain.pct 0 0 0 0. 0 0 0;
#P newex 727 65 245 196617 jmod.parameter.mxt $0_ source_select @type menu;
#B color 2;
#P objectname jmod.parameter.mxb[8];
#P newex 730 440 145 196617 jmod.message.mxt $0_ direct;
#B color 2;
#P objectname jmod.parameter.mxb[9];
#P newex 730 256 145 196617 jmod.parameter.mxt $0_ play;
#B color 2;
#P objectname jmod.parameter.mxb[5];
#P newex 730 339 145 196617 jmod.parameter.mxt $0_ loop;
#B color 2;
#P objectname jmod.parameter.mxb[4];
#P comment 58 230 101 196617 ---signal outputs---;
#P comment 27 283 79 196617 command input;
#N comlet (signal) Right Channel;
#P outlet 115 207 15 0;
#P outlet 8 358 15 0;
#N comlet (signal) Left Channel;
#P outlet 78 207 15 0;
#P inlet 8 283 15 0;
#P user umenu 192 103 63 196644 3 64 119 0;
#X setrgb 198 198 198 221 221 221 255 255 255 221 221 221 170 170 170 119 119 119 187 187 187;
#X add O0:O0:O0;
#P objectname Time_Display;
#P window setfont "Sans Serif" 12.;
#P comment 666 7 210 196620 All communication with interface is done using pvar;
#B color 7;
#P user radiogroup 193 70 42 16;
#X size 1;
#X offset 16;
#X inactive 0;
#X itemtype 1;
#X flagmode 0;
#X set 1;
#X done;
#P objectname Loop_toggle;
#P window setfont "Sans Serif" 9.;
#P comment 207 73 29 196617 loop;
#B frgb 181 181 181;
#P comment 732 644 214 196617 Fade in/out when audio is turned on/off is dealt with in the audio subpatch;
#P bpatcher 0 0 256 121 0 0 jmod.gui.mxt 0 $0_;
#P objectname jmod.gui.mxt;
#P comment 286 28 248 196617 The set message is important to prevent feedback into jmod.parameter.gain from killing ongoing ramps;
#P comment 487 148 131 196617 jmod.js_link.js takes care of linking gain parameters;
#P connect 8 0 101 0;
#P connect 101 0 10 0;
#P fasten 51 0 46 0 90 444 13 444;
#P fasten 50 0 46 0 52 444 13 444;
#P fasten 49 0 46 0 13 444 13 444;
#P hidden connect 34 0 97 0;
#P hidden connect 34 1 96 0;
#P connect 34 0 9 0;
#P connect 34 1 11 0;
#P connect 34 2 35 0;
#P connect 21 0 101 1;
#P connect 101 2 21 0;
#P fasten 86 0 87 0 291 104 280 104 280 57 291 57;
#P connect 87 0 86 0;
#P fasten 86 1 88 0 424 110 291 110;
#P fasten 70 0 85 0 291 249 279 249 279 151 291 151;
#P lcolor 12;
#P connect 88 0 85 0;
#P lcolor 12;
#P connect 85 0 100 0;
#P connect 100 0 75 0;
#P connect 75 0 70 0;
#P connect 70 0 81 0;
#P fasten 57 0 55 0 291 423 281 423 281 379 291 379;
#P connect 55 0 57 0;
#P fasten 55 1 61 0 450 428 291 428;
#P connect 61 0 59 0;
#P fasten 58 0 56 0 291 526 281 526 281 481 291 481;
#P connect 56 0 58 0;
#P fasten 56 1 62 0 450 532 291 532;
#P connect 62 0 60 0;
#P connect 64 0 65 0;
#P connect 65 0 63 0;
#P hidden connect 87 0 90 0;
#P hidden connect 75 0 76 0;
#P hidden connect 57 0 95 0;
#P hidden connect 58 0 94 0;
#P fasten 71 0 85 1 478 254 273 254 273 146 478 146;
#P lcolor 12;
#P connect 85 1 99 0;
#P connect 99 0 73 0;
#P connect 73 0 71 0;
#P connect 71 0 82 0;
#P connect 91 0 93 0;
#P hidden connect 73 0 77 0;
#P fasten 24 0 17 0 732 104 721 104 721 62 732 62;
#P connect 17 0 24 0;
#P fasten 17 1 25 0 967 108 732 108;
#P connect 33 0 84 0;
#P connect 84 0 31 0;
#P fasten 98 0 31 0 840 191 732 191;
#P fasten 26 0 15 0 735 298 726 298 726 252 735 252;
#P connect 83 0 15 0;
#P connect 15 0 26 0;
#P fasten 15 1 27 0 870 304 735 304;
#P fasten 28 0 14 0 735 381 727 381 727 335 735 335;
#P connect 14 0 28 0;
#P fasten 14 1 30 0 870 389 735 389;
#P connect 30 0 29 0;
#P connect 16 0 32 0;
#P fasten 69 0 80 0 737 620 727 620 727 555 737 555;
#P connect 80 0 72 0;
#P connect 72 0 69 0;
#P connect 69 0 79 0;
#P hidden connect 33 0 38 0;
#P hidden connect 26 0 36 0;
#P hidden connect 28 0 37 0;
#P hidden connect 24 0 39 0;
#P hidden connect 72 0 78 0;
#P connect 23 0 98 0;
#P pop;
