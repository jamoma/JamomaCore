max v2;
#N vpatcher 73 47 690 785;
#P window setfont "Sans Serif" 9.;
#P window linecount 2;
#P message 94 662 65 196617 \; max refresh;
#P window linecount 1;
#P newex 282 15 124 196617 r $0_xxx_to_filtergraph;
#P newex 177 285 89 196617 pvar FilterType 2;
#P newex 93 422 33 196617 atodb;
#P newex 16 638 124 196617 s $0_xxx_to_filtergraph;
#N vpatcher 14 59 611 388;
#P window setfont "Sans Serif" 9.;
#P window linecount 1;
#P newex 267 143 33 196617 dbtoa;
#P window linecount 0;
#P comment 401 232 119 196617 Set message to 6nd inlet;
#P outlet 96 274 15 0;
#P newex 96 232 215 196617 pak set 1200. 1. 1.;
#P newex 335 119 50 196617 route set;
#P inlet 199 50 15 0;
#P newex 267 119 50 196617 route set;
#P inlet 267 50 15 0;
#P newex 199 119 50 196617 route set;
#P inlet 335 50 15 0;
#P comment 405 121 100 196617 Get rid of set;
#P connect 7 0 8 0;
#P connect 2 0 7 1;
#P connect 1 0 2 0;
#P connect 10 0 7 2;
#P connect 3 0 4 0;
#P connect 4 0 10 0;
#P connect 6 0 7 3;
#P connect 5 0 6 0;
#P pop;
#P newobj 16 614 124 196617 p PrepareForFiltergraph~;
#P newex 177 469 75 196617 pvar FilterGain;
#P newex 177 393 39 196617 pvar Q;
#P newex 21 368 93 196617 pvar FilterGraph 7;
#P newex 177 532 83 196617 pvar CenterFreq;
#P message 105 115 43 196617 autodoc;
#P window linecount 5;
#P newex 2 144 164 196617 jmod.hub $0_ jmod.filterM~ @size 1U-half @module_type audio @num_inputs 1 @num_outputs 1 @description "Mono 2nd order IIR filter based on biquad~";
#P window linecount 1;
#P newex 259 235 115 196617 poly~ jmod.filterM~.alg;
#P comment 6 23 41 196617 type;
#B frgb 172 172 172;
#P window linecount 2;
#P message 18 662 72 196617 \; jmod.init bang;
#P window linecount 3;
#P newex 169 144 81 196617 pattrstorage @autorestore 0 @savemode 0;
#X client_rect 0 0 640 240;
#X storage_rect 0 0 640 240;
#P objectname jmod.filterM~;
#P window linecount 2;
#P hidden newex 282 39 270 196617 jmod.message $0_ filtergraph @description "Messages to filtergraph. See the MSP reference for further details";
#P objectname jmod.parameter.mxb[4];
#P flonum 118 31 40 9 0. 100. 8227 3 198 198 198 221 221 221 222 222 222 0 0 0;
#P objectname Q;
#P newex 177 490 305 196617 jmod.parameter $0_ filtergain @type msg_float @range -24 24 @clipmode both @ramp 1 @repetitions 0 @description "Gain (dB)";
#P objectname jmod.parameter.mxb;
#P newex 177 415 305 196617 jmod.parameter $0_ q @type msg_float @ramp 1 @range 0. 100. @clipmode both @repetitions 0 @description "Resonance (Q)";
#P objectname jmod.parameter.mxb[1];
#P newex 177 553 334 196617 jmod.parameter $0_ cf @type msg_float @range 30. 11025. @clipmode both @ramp 1 @repetitions 0 @description "Center frequency (Hz)";
#P objectname jmod.parameter.mxb[2];
#P user ubumenu 5 35 62 196617 0 1 1 0;
#X add lowpass;
#X add highpass;
#X add bandpass;
#X add bandstop;
#X add peaknotch;
#X add lowshelf;
#X add highshelf;
#X prefix_set 0 0 <none> 0;
#X pattrmode 1;
#P objectname FilterType;
#P window linecount 1;
#P comment 107 33 27 196617 q;
#B frgb 172 172 172;
#P flonum 118 44 40 9 -24. 24. 8227 3 198 198 198 221 221 221 222 222 222 0 0 0;
#P objectname FilterGain;
#P flonum 118 18 40 9 30. 11025. 8227 3 198 198 198 221 221 221 222 222 222 0 0 0;
#P objectname CenterFreq;
#P comment 103 20 18 196617 cf;
#B frgb 172 172 172;
#P comment 70 46 51 196617 filtergain;
#B frgb 172 172 172;
#P window setfont Helvetica 9.;
#P user filtergraph~ 165 20 88 39 21 9 20. 22050. 0.0625 16. 0 1 1 0 0 1 1;
#X frgb 99 99 99;
#X brgb 173 173 173;
#X rgb2 0 0 0;
#X rgb3 84 84 84;
#X rgb4 0 0 0;
#X rgb5 118 121 166;
#X rgb6 122 74 74;
#X rgb7 255 22 22;
#X linmarkers 5512.5 11025. 16537.5;
#X logmarkers 50. 500. 5000.;
#X nfilters 1;
#X setfilter 0 1 0 0 0 1200. 1. 1.2 30. 11025. 0.0625 16. 0. 100.;
#X done;
#P objectname FilterGraph;
#P window setfont "Sans Serif" 9.;
#P hidden comment 206 80 101 196617 ---signal output---;
#P comment 16 115 79 196617 command input;
#P hidden inlet 24 -21 15 0;
#P hidden comment 23 -35 105 196617 ----signal input----;
#P outlet 2 210 15 0;
#P hidden outlet 246 64 15 0;
#P inlet 2 115 13 0;
#P bpatcher 0 0 256 60 0 0 jmod.gui.mxt 0 $0_;
#P objectname jmod.gui.1Uh.a.mono.mxb;
#P window linecount 3;
#P newex 177 311 290 196617 jmod.parameter $0_ filtertype @type menu @description "What kind of filter to use. Possible values: lowpass | highpass | bandpass | bandstop | peaknotch | lowshelf | highshelf";
#P objectname jmod.parameter.mxb[3];
#P hidden connect 6 0 1 0;
#P lcolor 6;
#P connect 2 0 25 0;
#P fasten 26 0 25 0 110 137 7 137;
#P connect 25 0 4 0;
#P connect 17 0 31 0;
#P connect 31 0 32 0;
#P fasten 0 0 28 0 182 361 26 361;
#P connect 18 0 31 1;
#P connect 28 2 33 0;
#P connect 16 0 31 2;
#P connect 21 0 25 1;
#P hidden connect 20 0 9 0;
#P connect 25 2 21 0;
#P fasten 0 0 34 0 182 356 171 356 171 281 182 281;
#P fasten 34 1 0 0 261 307 182 307;
#P fasten 17 0 29 0 182 448 170 448 170 388 182 388;
#P connect 29 0 17 0;
#P connect 28 3 17 0;
#P fasten 18 0 30 0 182 524 170 524 170 466 182 466;
#P connect 30 0 18 0;
#P connect 33 0 18 0;
#P fasten 16 0 27 0 182 588 171 588 171 528 182 528;
#P connect 27 0 16 0;
#P connect 28 1 16 0;
#P hidden connect 35 0 9 5;
#P hidden fasten 24 0 1 1 264 258 567 258 567 -8 251 -8;
#P hidden connect 1 1 3 0;
#P hidden fasten 1 0 24 0 5 95 264 95;
#P lcolor 6;
#P fasten 25 1 24 0 84 218 264 218;
#P pop;
