...
void param_clip_list(t_param *x)
{
	...
	for(i=0; i < x->list_size; i++){
		if(x->atom_list[i].a_type == A_LONG){
			if(x->attr_clipmode == ps_low)
				x->atom_list[i].a_w.w_long = tt_audio_base::limit_min(x->atom_list[i].a_w.w_long, (long)x->attr_range[0]);
			else if(x->attr_clipmode == ps_high)
				x->atom_list[i].a_w.w_long = tt_audio_base::limit_max(x->atom_list[i].a_w.w_long, (long)x->attr_range[1]);
			else if(x->attr_clipmode == ps_both)
				x->atom_list[i].a_w.w_long = tt_audio_base::clip(x->atom_list[i].a_w.w_long, (long)x->attr_range[0], (long)x->attr_range[1]);
		}
		else if(x->atom_list[i].a_type == A_FLOAT){
			if(x->attr_clipmode == ps_low)
				x->atom_list[i].a_w.w_float = tt_audio_base::limit_min(x->atom_list[i].a_w.w_float, x->attr_range[0]);
			else if(x->attr_clipmode == ps_high)
				x->atom_list[i].a_w.w_float = tt_audio_base::limit_max(x->atom_list[i].a_w.w_float, x->attr_range[1]);
			else if(x->attr_clipmode == ps_both)
				x->atom_list[i].a_w.w_float = tt_audio_base::clip(x->atom_list[i].a_w.w_float, x->attr_range[0], x->attr_range[1]);			
		}
	}
}
