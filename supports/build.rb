...
if(ARGV.length == 0)
  puts "usage: "
  puts "build.rb <required:configuration> <optional:clean> <optional:debug> <optional:version> <optional:revision> <optional:distropath>"
  exit 0;
end
...
use_make = generate_makefile("#{@svn_root}/library", "Jamoma#{@projectName}", "..")

if win32?
	build_project("#{@svn_root}/library", "Jamoma#{@projectName}.vcproj", configuration, true, @distropath)
elsif linux?
	build_project("#{@svn_root}/library", "Makefile", configuration, true, "#{@distropath}/core")
	puts `sudo cp #{@svn_root}/library/build/*.so /usr/lib`
else
  if (@distropath)
	  build_project("#{@svn_root}/library", "Jamoma#{@projectName}.xcodeproj", configuration, true, "#{@distropath}/Libraries", use_make)
	else
	  build_project("#{@svn_root}/library", "Jamoma#{@projectName}.xcodeproj", configuration, true, nil, use_make)
...
zero_count
if @distropath
  build_dir("extensions", configuration, clean, "#{@distropath}/Extensions")
else
  build_dir("extensions", configuration, clean, "/Library/Application Support/Jamoma/Extensions")
  if linux?
...
if (@distropath == nil && !linux?) # if a custom distropath is defined, don't build the Max externs (they probably aren't wanted so just build the frameworks and extensions)
  ...
  zero_count
  build_dir("implementations/MaxMSP", configuration, clean, nil)
  ex_total, ex_count = get_count
  puts ""

  if @projectName == "Modular"
    build_dir("implementations/MaxMSP-core", configuration, clean, nil)
    ex_total, ex_count = get_count
    puts ""

    build_dir("implementations/MaxMSP-utilities", configuration, clean, nil)
    ex_total, ex_count = get_count
    puts ""
  end
