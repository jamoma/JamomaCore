...
typedef struct _wrappedModularInstance {
	...
	AtomCount								argc;						
	AtomPtr									argv;
	TTString								*i_format;					///< a format string to edit numeric instance when we use internals as an array of wrappedObject
	TTString								*s_format;					///< a format string to edit string instance when we use internals as an array of wrappedObject
	...
	TTSymbolPtr								patcherName;				///< the patcher name in which the external is
	...
	TTSymbolPtr								cursor;						///< to select an entry in x->internals
...
TTErr		removeInternals_data(TTPtr self, TTNodeAddressPtr address, TTSymbolPtr name);
