...
TTErr wrapTTClassAsMaxUIClass(TTSymbolPtr ttblueClassName, char* maxClassName, WrappedClassPtr* c, WrappedClassOptionsPtr options)
{
	...
	for (TTUInt16 i=0; i<v.getSize(); i++) {
		...
		hashtab_store(wrappedMaxClass->maxNamesToTTNames, nameMaxSymbol, ObjectPtr(name));
		class_addattr(wrappedMaxClass->maxClass, attr_offset_new(nameMaxSymbol->s_name, maxType, 0, (method)wrappedUIClass_attrGet, (method)wrappedUIClass_attrSet, NULL));
