...
if win32?
  ...
  def create_test_logs
    # set up log files and ensure that the build_root is there
    `mkdir -p #{@log_root}` if !FileTest.exist?(@log_root)
    @test_log = File.new("#{@log_root}/test.log", "w")
    @test_log.write("JAMOMA TEST LOG: #{`date`}\n\n")
    @test_log.flush
    trap("SIGINT") { 
      puts "Crash!"
      @test_log.close
  ...
  def generate_makefile(projectdir, projectname, path_to_moduleroot="../..")
    ...
    if ($g_use_yaml_project_files && File.exists?("#{projectdir}/#{projectname}.yml"))
      ...
      if win32?
        ...
        if mac?
          ...
          makefile.write("LDFLAGS = $(LIBS)\n")
          makefile.write("LDFLAGS += -install_name \"@executable_path/../Jamoma/lib/$(NAME).dylib\" \n") if project_type == "library"
        end
  ...
  def find_and_build_project(projectdir, configuration, clean, distropath)
    foldername = projectdir.split("/").last
    use_make = generate_makefile(projectdir, foldername)
  ...
  def build_dir(dir, configuration, clean, distropath)
    dir = "#{@svn_root}/#{dir}"
    ...
    Dir.foreach(dir) do |subf|
      ...
      find_and_build_project("#{dir}/#{subf}", configuration, clean, distropath)
    end
  end   
