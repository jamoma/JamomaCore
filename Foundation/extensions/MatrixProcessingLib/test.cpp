...
extern "C" TTErr TTLoadJamomaExtension_MatrixProcessingLib(void);               // CUSTOMIZE

int main(int argc, const char * argv[])
{
	std::cout << "BEGIN testing for MatrixProcessingLib...\n";                  // CUSTOMIZE
    TTLoadJamomaExtension_MatrixProcessingLib();                                // CUSTOMIZE
	
	TTValue classNames;
	
	// if the follow group tag is present within the thisTTClassTags definition, the class will be tested
    TTValue testClassesWithTheseTags(TT("foundationMatrixProcessingLib"));             // CUSTOMIZE
	...
	std::cout << "END testing of MatrixProcessingLib!\n";                       // CUSTOMIZE
    return 0;
}
