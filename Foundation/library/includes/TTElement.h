...
#pragma mark assignment
    ...
    TTElement& operator = (const TTElement& anOtherValue)
    {
        ...
        else if (anOtherValue.mType == kTypeString)
            mValue.stringPtr = new TTString(*anOtherValue.mValue.stringPtr);
        else
...
#pragma mark transformation
    ...
    void fold(const TTFloat64& lowBound, const TTFloat64& highBound)
    {
        if (TTDataInfo::getIsNumerical(mType)) {
            // TODO: find a way to make this routine faster
            switch (mType) {
    ...
    void wrap(const TTFloat64& lowBound, const TTFloat64& highBound)
    {
        if (TTDataInfo::getIsNumerical(mType)) {
            // TODO: find a way to make this routine faster
            switch (mType) {
