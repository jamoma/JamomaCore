...
TTObjectBase::~TTObjectBase()
{
    ...
    messages->iterate(this, [] (TTPtr, const TTKeyVal& it) {
            delete (TTMessagePtr)(TTPtr(it.second));
        });
    delete messages;

    // Delete attribute objects, then delete the hash that maintains them.

    attributes->iterate(this, [] (TTPtr, const TTKeyVal& it) {
            delete (TTAttributePtr)(TTPtr(it.second));
        });
    delete attributes;
}
