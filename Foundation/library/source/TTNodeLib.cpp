...
void TTNodeLibInit()
{
    ...
    ttRegexForDirectory.reset(new TTRegex(R"(([-\w]+):/)"));
    ttRegexForAttribute.reset(new TTRegex(":+", 0));
    ttRegexForParent.reset(new TTRegex(R"((.*)/+(\S+))"));
    ttRegexForInstance.reset(new TTRegex("[.]", 0));
    ttRegexForInstanceZero.reset(new TTRegex(R"(\.0)", 0));
