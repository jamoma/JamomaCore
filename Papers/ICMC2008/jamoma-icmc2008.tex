...
A property is an aspect of the parameter which itself has a state. For example, if repetition filtering is turned on or turned off. A method is simply a mechanism for doing something, such as refreshing the user interface for the parameter, but does not have any value to maintain. Often when we refer to properties, we are collectively referring to both the properties and the methods of a parameter.
...
Jamoma's parameter object is an implementation of the idea that properties and methods can meaningfully extend parametric control. Table~\ref{tab:parameter_properties} lists the currently implemented properties of this parameter object as addressed using the Open Sound Control protocol \cite{Wright:2003}. The underlying details of how this is implemented are detailed in Section~\ref{ssec:ramplib}.
