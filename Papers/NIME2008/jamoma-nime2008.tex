...
The paper suggests an approach for creating structured Open Sound Control (OSC) messages by separating the addressing of node values and node properties.  This includes a mechanism for querying values and properties.  As a result, it is possible to address nodes in more complex tree structures using an OSC namespace.  This is particularly useful for creating flexible communication in modular systems.  A prototype implementation is presented and discussed.
...
Open Sound Control (OSC)\footnote{http://www.opensoundcontrol.org} is a protocol for transmitting messages that address nodes using a familiar "slash" notation.  It has been adopted by the computer music community as a defacto standard for comminication both within a process and between separate processes.  OSC does not prescribe any particular namespace standardization, only a message formatting standardization.

The authors have been involved in developing OSC namespaces for the Jamoma\footnote{http://www.jamoma.org} project.
Jamoma is a modular system for developing high-level modules in the Max/MSP/Jitter environment \cite{Place:2006}.  Communication in and between Jamoma modules is being handled through the Open Sound Control protocol. 

As the modular structures in Jamoma grew more complex, the authors found the flat namespace convention of OSC message formatting to be inadequate for addressing our constructs.  Specifically, we found OSC to be ideal for specificying an address to a node.  However, it became increasing unclear what to do once the address reached this said node.  The problem is excerbated when the node itself implements its own OSC namespace.
...
There are a number of projects ongoing which attempt to standardize namespaces for various uses of Open Sound Control.  These include...
...
Open Sound Control 2.0 suggestion -- Developers of the Lemur multitouch interface presented a draft for a 2.0 version of Open Sound Control during an OSC developer meeting at NIME 2006 \cite{Jazzmutant:2006}, and has later changed the suggestion slightly \cite{Jazzmutant:2007}. 

The topic of query has been brought up and been suggested as follows: 

    OSCQS \cite{Habets:2005}
    ...
    Copperlan
    McGill Mapper \cite{Malloch:2007}
    SpatDIF
    Integra
    OSC2
...
In actual practice, these various independent efforts at standardzing namespaces incorporate syntactic elements with conflicting meanings as compared to one another.  There are some commonalities to these efforts and the problems that they try to address.  Specifically:
...
It is clear from a review of these sundry efforts that additional syntax is needed for clarifying function, address, or both when developing a complex OSC namespace.  Some examples...  

The problem of accessing properties of a node, as opposed to the value of a node, is particularly troublesome using the standard OSC addressing conventions (using only slashes to navigate the tree).  One example of system parallel to OSC for representing and sending data over a network is XML.
...
XML is a particularly relevant analogue to OSC.  XML defines a means for formatting data, but not the data or the anything specific to the dataspace itself CITATION.  A number of standardized namespaces using XML have gained wide adoption, including SVG and XHTML and MAYBE SOAP(?).
