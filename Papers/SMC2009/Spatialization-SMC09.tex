...
The examples of work on spatialization done at ICST and in Jamoma and HoloEdit illustrates that a stratified model can be fruitful for development within media programming environments. The modular framework TANGA \cite{ReiterTANGA} for interactive audio applications reveals a related separation of tasks.
...
One keystone may be to define a meaningful and accepted interchange communication format for spatialization. Therefore further work needs to be done on SpatDIF which will culminates in an API that easily integrates in any spatialization software.
