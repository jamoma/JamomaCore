...
When dealing with spatialisation in electroacoustic composition or linear sound editing, the workflow comprises a number of steps in order to construct, shape and realize the spatial qualities of the work. The creative workflow might appear to be different when working on audio installations or interactive/multimedia work. Still we find that there are underlying common elements that are always in play when spatialization is used. For this reason a layered approach is proposed, where the processes involved are organized according to levels of abstraction.

This model gets its inspiration from the OSI network model\footnote{\url{http://en.wikipedia.org/wiki/OSI_model}}, which is an abstract description for layered communications and computer network protocol design. OSI (Open Systems Interconnection) divides network architecture into seven layers that range from top to bottom between the Application and Physical Layers. Each OSI-layer contains a collection of conceptually similar functionalities that provide services to the layer above it and receives service from the layer below it. The adaptation of concepts originally designed for network protocols to computer music systems was legitimized, for instance, in creating the popular Open Sound Control (OSC) protocol \cite{wright1997osc}.

In our proposed model, depicted in Figure \ref{fig:layers}, six layers have been identified.

	\begin{figure}
		[ht] \centerline{\framebox{ 
		\includegraphics[width=0.95\columnwidth]{layers2.pdf}}} \caption{Processing layers and communication stream} \label{fig:layers} 
	\end{figure}
...
Not every rendering technique generate intermediate encoded signals, but instead can be considered to encapsulate the encoding and decoding layers in one process. Some examples of such renderes are VBAP \cite{Pulkki:1997vbap}, DBAP \cite{dbapICMC09}, ViMiC \cite{CMJ08-VIMIC} and Ambisonics equivalent panning \cite{Neukom:2008ambipan}.

Processing of sources to create an impression of distance, such as Doppler effect, gain attenuation and air absorbtion filters, are considered to belong to the encoding layer. Likewise early reflections and reverberation belongs in the encoding layer. The Waves IR360 surround convolution reverb internally use B-format reverb impulse responses.
