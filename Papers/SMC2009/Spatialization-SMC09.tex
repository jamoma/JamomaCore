...
\begin{abstract}
The improvements in computer and audio equipment in recent years makes it possible to experiment more freely with resource-demanding sound synthesis techniques, such as sound spatialization. For seeking new ways of expressions, different spatialization application should be easy to combined and accessible from one generic user interface and/or other controllers. %e.g. a generative mapping of a granular synth output to different spatial renderer from one common user interface, while also streaming spatial encoded audio to the internet. 
To guarantee an efficient workflow for such non-standardized approaches structure, flexibility, and interoperability are required. Common spatialization systems are often too self-contained and do not consider these requirements. Therefore we proposes a multi-layer concept to mediate sound spatialization.\\                              	
...
For instance, an interactive art installation where the real-time quality of a spatial rendering system is of great importance in combination with the possibility to control spatial processes through a multi-touch screen, whereas for a performance of a fixed-media composition features such as multichannel playback and the compensation of non-equidistant loudspeaker in terms of sound pressure and time delays may be paramount. Other requirements may be binaural rendering for headphone listening, multichannel recording, up-and down mixing, or a visual representation of a sound scene.  
Moreover, even during the creation of one spatial art work, the importance of these requirements change throughout the different stages of the creative processes. % \emph{Experimentation -- Arranging -- Performance -- Documentation}. 
...
Moreover, the 5.1 setup favors frontal direction and has limited capabilities for localizing virtual sources from the sides and back which may limit its artistic use. More recent, extensions to 7.1 or 10.2 can be found\footnote{A comparison of DAWs concerning their multichannel audio capabilities shows \url{http://acousmodules.free.fr/hosts.htm}.}.   
