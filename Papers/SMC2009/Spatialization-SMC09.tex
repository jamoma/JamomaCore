...
In the proposed model the spatial rendering is considered to consist of two layers. The encoding layer produce encoded signals containing spatial information while being independent of and unaware of the speaker layout. The decoding process interprets the encoded signal and decodes it for the speaker layout at hand. \cite[p.~99]{Wiggins2004PhDThesis} arguments for the benefits of such hierarchical rendering methods:
...
Examples of such hierarchical rendering methods are Directional Audio Coding (DIRAC) \cite{Pulkki2007dirac_1}, Ambisonics B-Format, Higher-order Ambisonics, MPEG Surround, DTS, and AC3.
Not every rendering technique generate intermediate encoded signals: for instance VBAP \cite{Pulkki:1997vbap}, DBAP \cite{dbapICMC09}, ViMiC \cite{CMJ08-VIMIC}, or Ambisonics equivalent panning \cite{Neukom:2008ambipan} encapsulate the encoding and decoding layers in one process.
