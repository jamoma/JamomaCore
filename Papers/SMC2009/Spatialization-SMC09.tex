...
\begin{abstract}
The availability of better CPU resources in recent years makes it possible to experiment more freely with spatial rendering for seeking new ways of expressions, e.g. a generative mapping of a granular synth output to different spatial renderer from one common user interface, while also streaming spatial encoded audio to the internet. To guarantee an efficient workflow for such non-standardized approach, structure and flexibility are required. This paper proposes a multi-layer concept to meet these requirements.\\                              	
\textbf{Keywords:} sound spatialization, interoperability, layered architecture, adaptation, site-specific, Ambisonics      
