...
Moreover, a number of spatial effects processors (Doppler, Airfilter, RollOff, Reverb), multichannel recording \& playback modules, and other utilities to facilitate the work with multichannel audio (loudspeaker setup definition tools including sound-pressure and delay compensation for non-equidistant arrangements, multichannel auxiliaries, level-meters) are part of Jamoma.
All these modules have been designed according to a common namespace model, that enables the user to rapidly connect or exchange modules. In particular, the interoperability of the described Holo-Edit and ICST Ambimonitor with physical interfaces, such as Lemur\footnote{\url{http://www.jazzmutant.com}} or TUIO-compliant \cite{kaltenbrunner2005tpt} multitouch devices\footnote{We apply the Max Multitouch Framework \url{http://code.google.com/p/mmf}.}) facilitates to create individual mappings to Jamoma's rendering modules.\\
