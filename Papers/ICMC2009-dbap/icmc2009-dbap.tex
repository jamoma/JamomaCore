...
% CHANGED: Tim capitalized the DBAP words -- if not desired, feel free to undo it
...
% CHANGED: Tim reworded this paragraph very significantly -- please check!
% TODO: Tim also says... Due to a conflict in SVN, I can't tell what happened here -- maybe this paragraph was just deleted ?
...
% TODO: Visualization is the other spelling of this word -- spelled with an s is the British spelling, which might be okay.
...
% TODO: Tim notes introduction of new terminology with no precedent.  What is a "view matrix"?  What is a "hitmap"  And what is "digs"?
% TODO: Tim thinks the last sentence of this paragraph doesn't make much sense and is very difficult to read.  All of the "ing" words bunched together is one problem.

For each pixel of a selected view matrix, the pixel value is set to the squared amplitude returned by the dbap algorithm if the source was located at this position. The visual result in figure \ref{fig:1spk} indicates the spatial response for one of the loudspeakers. The lighter pixels are positions where amplitudes are close to $1$ and the darker ones are positions where amplitudes are close to $0$. As expected, the further away the source is from the loudspeaker, the lower the amplitude. However the presence of ``digs'' in the spatial distribution of the amplitude response illustrates how the DBAP algorithm works when a source moves between several loudspeakers. The graphical information provides the user with a kind of hitmap assisting in space scripting and performing.

% TODO: Tim may not know what he is talking about, but he thinks the numbers for the dB values look odd in the PDF.  The 6 dB in particular.  Ignore if you think he's nuts.
...
% TODO: Tim worked on some minor wording issues below , but still isn't 100% sure what the first sentence means and also doesn't understand the second sentence at all.

A visualisation of the response ranges for all speakers can be created by combining all figures of the same kind as fig. \ref{fig:1spk}, and then finding the maximum amplitude for 2D-position. The graphical result shows straight segmented parts in space corresponding to every spatial influence zones of loudspeakers. Here the darker pixels have to be explained as positions where the source is mixed equally between the closest loudspeakers. Figures \ref{fig:allspk1}, \ref{fig:allspk2} and \ref{fig:allspk3} illustrate several settings of varying roll off and blur values. 
...
% TODO: Discuss ViMiC as a more versatile and CPU-hungry alternative
% TODO: DBAP for sound FX processing, references to Ali Momeni and Hipnoscope
% TODO: DBAP for gaming: Traversing through a virtual soundscape by moving the speaker positions
