...
% TODO: Add reference to Ville Pulkkis paper, and rewrite this section to get better flow.
...
In order to allow intuitive understanding and manipulation of the algorithm for the users, the authors developped a visual representation of the behaviour of the algorithm, using gradients to represent the "predominance" of each speaker according to the signal spreading. This is intended to give a more intuitive manipulation of source positions, as well as managing parameters such as spatial blur, rolloff, or spatial weights, that can sound rather abstract to the non-scientific.


For each pixel of a selected view matrix (of X * Y pixels), the pixel value is set to the  squared amplitude returned by the dbap algorithm as if a source was at this position. The visual result is a picture that show to the user the spatial response of  a loudspeaker. The lighter pixels are positions where amplitudes is closed to 1. and the darker ones, positions where amplitudes are closed to 0.. As expected, the far the source is from the loudspeaker, the less is the amplitude. However the presence of “digs” in the spatial distribution of the amplitude response illustrates how the dbap algorithm works when a source moves between several loudspeakers. Those graphical informations, even esthetic there are, give to the user a kind of hitmap that could assists him during space writing and performance time.


But those pictures of spatial response of each loudspeaker need to be shown together in one visualization. The layout “fusion” method chosen is a max operation between each pixel of each pictures. The graphical result show straight segmented parts in space corresponding to every spatial influence zones of loudspeakers. Here the darker pixels have to be explain as positions where the source is equal mixed between the closest loudspeakers. The following pictures illustrate several settings of rolloff and blur parameters on all loudspeakers. The last picture illustrates a weights preset that keep only five loudspeakers set with different weights.
...
%TODO : turn all equations in TeX format !
