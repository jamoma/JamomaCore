...
void jamoma_init(void)
{
	if(!initialized){
		...
		object_method_parse(max, SymbolGen("definecommand"), "I patcher insertobj bpatcher @name jmod.input~.maxpat @args /input~", NULL);
		// O -- Output: a new audio output module	
		object_method_parse(max, SymbolGen("definecommand"), "O patcher insertobj bpatcher @name jmod.output~.maxpat @args /output~", NULL);
	
		// B -- BPatcher: a new module in a bpatcher
		object_method_parse(max, SymbolGen("definecommand"), "B patcher inserttextobj \"bpatcher @name jmod. @args myModule\"", NULL);		
		...
		object_method_parse(max, SymbolGen("definecommand"), "D patcher inserttextobj \"bpatcher @name jmod.\"", NULL);		
