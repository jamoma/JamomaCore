...
void hub_internals_create(t_hub *x)
{
	...
	anObject = new hubInternalObject("jcom.message", 	"preset/read",				x->container,	"msg_symbol",	"none",	"Open an xml-preset file and recall the first preset in that file.  An optional argument defines the file to open.", 1);
	anObject->action = (method)hub_preset_read;
	hashtab_store(x->hash_internals, gensym("preset/read"), (t_object*)anObject);

	anObject = new hubInternalObject("jcom.message", 	"preset/write",				x->container,	"msg_symbol",	"none",	"Write an xml-preset file to disk.  An optional argument defines the file to open.", 1);
	anObject->action = (method)hub_preset_write;
	hashtab_store(x->hash_internals, gensym("preset/write"), (t_object*)anObject);

	anObject = new hubInternalObject("jcom.message", 	"preset/writeagain",		x->container,	"msg_symbol",	"none",	"Write on same xml-preset file.", 1);
