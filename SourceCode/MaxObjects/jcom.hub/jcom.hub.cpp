...
t_symbol* hub_subscribe(t_hub *x, t_symbol *name, t_object *subscriber_object, t_symbol *type)
{
	...
	if(x->osc_name != _sym_nothing){
		...
		jamoma_directory_register(gensym(fullAddress), type, subscriber_object, &newNode, &newInstanceCreated);
...
void hub_receive(t_hub *x, t_symbol *name, long argc, t_atom *argv)
{
	...
	err = jamoma_directory_get_node(gensym(oscAddress), &returnedNodes, &firstReturnedNode);
...
/*			TODO :This has to happen only when setting the OSC name from the module's file name
		...
		newInstanceCreated = false;
		jamoma_directory_register(nameOriginal, gensym("hub"), (t_object *)x, &newNode, &newInstanceCreated);
