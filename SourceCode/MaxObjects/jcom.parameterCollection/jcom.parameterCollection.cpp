...
void paramarray_add(t_paramarray* x, long i_add)
{
	...
	if(i_add > 0){
		...
		if(new_size >= 0){
			s_num = (char *)malloc(sizeof(char)*256);
			snprintf(s_num, 256, "%s.[%ld]", x->attr_name->s_name, new_size);
			atom_setsym(a, gensym(s_num));
			defer((t_object*)x,(method)paramarray_create_parameter, NULL, 1, a);
			free(s_num);
		}
	}
}

void paramarray_remove_parameter(t_paramarray* x, long i_rm)
{
	long ht_size, new_size;
	char *s_num;
	t_atom a[1];
